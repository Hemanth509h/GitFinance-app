...
function WorkEntryForm({ initialData, onSubmit, onCancel }: any) {
    const [data, setData] = useState(() => ({ date: initialData?.date?.slice(0, 10) || formToday(), client: initialData?.client || "", amount: String(initialData?.amount || ""), amountPaid: String(initialData?.amountPaid || "0"), status: initialData?.status || "Unpaid", datePaid: initialData?.datePaid?.slice(0, 10) || "", description: initialData?.description || "" }));
