...
function ExpenseForm({ initialData, onSubmit, onCancel }: any) {
    const [data, setData] = useState(() => ({ date: initialData?.date?.slice(0, 10) || formToday(), category: initialData?.category || "Other", amount: String(initialData?.amount || ""), merchant: initialData?.merchant || "", paymentMethod: initialData?.paymentMethod || "Cash", description: initialData?.description || "" }));
    ...
    const submit = async () => { if (!data.date || Number(data.amount) <= 0) return setError("Enter a valid date and amount greater than zero."); setError(""); setSubmitting(true); try { await onSubmit({ ...data, amount: Number(data.amount), merchant: data.merchant.trim(), description: data.description.trim() }); } finally { setSubmitting(false); } };
    return <><FormField label="Expense date *" error={error}><FormInput value={data.date} onChangeText={(v) => set("date", v)} placeholder="YYYY-MM-DD" /></FormField><FormField label="Amount *"><FormInput keyboardType="decimal-pad" value={data.amount} onChangeText={(v) => set("amount", v)} placeholder="Amount spent" /></FormField><FormField label="Merchant"><FormInput value={data.merchant} onChangeText={(v) => set("merchant", v)} placeholder="Shop, vendor, or person" /></FormField><FormField label="Category *"><FormChoices value={data.category} options={EXPENSE_CATEGORIES} onChange={(v) => set("category", v)} /></FormField><FormField label="Payment method"><FormChoices value={data.paymentMethod} options={PAYMENT_METHODS} onChange={(v) => set("paymentMethod", v)} /></FormField><FormField label="Description"><FormInput multiline value={data.description} onChangeText={(v) => set("description", v)} placeholder="Expense notes" style={{ height: 82, textAlignVertical: "top", paddingTop: 12 }} /></FormField><FormActions submitLabel={initialData ? "Update expense" : "Add expense"} onSubmit={submit} onCancel={onCancel} submitting={submitting} /></>;
}
...
export default function Expenses() {
    ...
    const isCurrentMonth = new Date().toISOString().substring(0, 7) === selectedMonth;
                ...
                >
                    ...
                    <View style={styles.metricsGrid}>
                        ...
                        <MetricTile
                            ...
                            subtext={topCategoryAmount > 0 ? formatCurrency(topCategoryAmount) : "No spend"}
                                ...
                                {PAYMENT_METHODS.map((method) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            ...
                                            paymentMethodInput === method && styles.statusSelectorBtnActive,
                                        ]}
