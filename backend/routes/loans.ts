...
router.post("/", async (req, res) => {
  const { lenderName, totalAmount, monthlyInterest, startDate, status } =
    req.body;
  try {
...
router.get("/:id/repayments", async (req, res) => {
  try {
    ...
    const repayments = await Repayment.find({ loanId: req.params.id })
      .sort({ date: -1 })
      .populate("workEntryId", "client date amount description");
    res.json(repayments);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if (monthlyInterest !== undefined)
      loan.monthlyInterest = parseFloat(monthlyInterest) || 0;
    ...
    if (totalAmount !== undefined) {
      ...
      const interestAccrued =
        principalAmount > 0 ? loan.totalAmount - principalAmount : 0;
