...
interface LoanRepaymentInput {
  ...
  note?: string;
  method?: string;
  workEntryId: Types.ObjectId;
}

const applyLoanRepayment = async ({
  userId,
  loanId,
  amount,
  date,
...
router.get("/", async (req, res) => {
  try {
    const entries = await WorkEntry.find({ userId: req.user._id }).sort({
      date: -1,
    });
    res.json(entries);
  } catch (error) {
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const newEntry = new WorkEntry({
      ...
      amountPaid,
      datePaid:
        status === "Paid" ? req.body.datePaid || new Date() : req.body.datePaid,
      description,
    });
    await newEntry.save();
    ...
    if (loanId && loanRepaymentAmount) {
      await applyLoanRepayment({
        ...
        date: date || new Date(),
        note:
          loanRepaymentNote || `Repayment created from work log for ${client}`,
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if (
      ...
      (!updateData.amountPaid || updateData.amountPaid === 0)
    ) {
    ...
    if (loanId && loanRepaymentAmount) {
      await applyLoanRepayment({
        ...
        note:
          loanRepaymentNote ||
          `Repayment created from work log for ${updatedEntry.client}`,
        method: loanRepaymentMethod || "Work Log",
...
router.delete("/:id", async (req, res) => {
  try {
    const entry = await WorkEntry.findOneAndDelete({
      _id: req.params.id,
      userId: req.user._id,
    });
    if (!entry) return res.status(404).json({ message: "Entry not found" });
